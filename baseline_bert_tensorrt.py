...
for batch in dataloader:
    ...
    static_shape = sample_input_ids.shape  # e.g., torch.Size([8, 384])
    break

def model_wrapper(input_ids, attention_mask, token_type_ids):
    return model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)

# Compile with static input shape
def compile_trt_model_static(fp16_mode=True):
    try:
        print(f"Using static input shape: {static_shape}")
